...
const burger = (props: TheseProps) => {
  ...
  const transformedIngredientsValues: number[] = Object.values(
    props.ingredients
  );
  const ingredientsJsxElements: JSX.Element[] = [];

  if (
    transformedIngredientsValues.reduce((prev, current) => prev + current) === 0
  ) {
  ...
  ingredientsEmtyArray.forEach((arr) => {
    arr.map((ing, i) => {
      return ingredientsJsxElements.push(
